...
{
	if (expected->shift_spec != output->shift_spec)
		return false;
	...
	return shift_val_match && expected->shift_type == output->shift_type && expected->rm == output->rm;
}

static bool compare_data_processing(data_processing_t *expected, data_processing_t *output)
{
	if (expected->imm_const ^ output->imm_const)
		return false;
	...
	return operand2_decoded_correctly && expected->rn == output->rn && expected->rd == output->rd && expected->opcode == output->opcode && expected->set == output->set;
}
static bool compare_multiply(multiply_t *expected, multiply_t *output)
{
	return expected->rm == output->rm && expected->rd == output->rd && expected->rs == output->rs && expected->accumulate == output->accumulate && expected->set == output->set;
}
static bool compare_data_transfer(data_transfer_t *expected, data_transfer_t *output)
{
	if (expected->imm_offset ^ output->imm_offset)
		return false;
	...
	return offset_decoded_correctly && expected->rn == output->rn && expected->rd == output->rd && expected->imm_offset == output->imm_offset && expected->pre_index == output->pre_index && expected->up_bit == output->up_bit && expected->load == output->load;
}
